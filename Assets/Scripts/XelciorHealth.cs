...
public class XelciorHealth : MonoBehaviour
{
    [System.Serializable]
    ...
    public void TakeDamage(int damage)
    {
        if (!isBossDead)
        {
            ...
            if (!flashActive)
            {
                if (isEnrage)
                {
                    anim.Play("XelciorEnrageHurt");
                    GetComponent<Animator>().SetBool("Enrage Hurt", true);
                }
                else
                {
                    anim.Play("XelciorHurt");
                }
                stats.currentHealth -= damage;
                flashActive = true;
                bossDamage = true;
                flashCounter = flashLength + 1.5f;
                sfxMan.finalBossHurt.Play();

                if (stats.currentHealth <= 1000 && !isInvulnerable && !isEnrage)
                {
                    isEnrage = true;
                    sfxMan.bossChargeUP.Play();
                    sfxMan.finalBossEnrage.Play();
